...
| **B: duplicates** | `exclude_duplicates()` plan/execute. Richer plan struct (group count, skip categories: no-copy, multiple, in-prefer, unhashed). Already uses `domain::exclusion::find_excludable_duplicates()`. | Pending | — |
| **C: object-level + single-target** | `set_objects_by_filter()` plan/execute (different ceremony: dry-run-default, `--yes` to execute). Plus five thin single-target operations (`set_by_id`, `set_by_path`, `set_object_by_hash`, `set_object_by_file`, `clear_object`) and read-only `list_objects()`. | Pending | — |
