...
- `source.rs` - Source batch fetching and writes (`batch_fetch_by_roots()`, `fetch_sources_by_object_ids()`, `insert_destination()`, `apply_reconciliation()`, `sources_exist_at_scope()`, `fetch_decision_id_at_path()`, `set_excluded()` / `batch_set_excluded()` (flip `excluded` and write `decision_id`; `None` → NULL), `set_decision_id_by_object()` (stamp `decision_id` on every source sharing an object))
...
- `exclude.rs` - Exclude plan/execute: `plan_set()`, `execute_set()`, `plan_clear()`, `execute_clear()`, `plan_duplicates()`/`execute_duplicates()`, `plan_set_objects()`/`execute_set_objects()`, single-target `check_*`/`execute_set_source`/`execute_set_object`/`execute_clear_object`. `ExcludeItemData` is the receipt-capable plan source-of-truth (root, rel_path, hash, size, mtime, previous_decision_id); plans expose `source_ids()`/`paths()` accessors. `DuplicateGroupData` carries kept/excluded groups. Execute fns take `placement: Option<&ReceiptPlacement>`, set `decision_id` (via `batch_set_excluded`/`set_decision_id_by_object`), build the matching receipt body, and drain warnings into the result struct.
...
- `decision.rs` - Decision recording: `DecisionRecorder` (two-phase start/complete), `DecisionParams` (`record_enabled`, `receipt_enabled`, `ledger_config`), `DecisionCounts`. `DecisionRecorder::start()` signature: `(conn, params, placement: Option<&ReceiptPlacement>)` (the placement abstraction lives in `receipt.rs`). Shared receipt primitives (used by both exclude and apply): `DecisionParams::receipt_meta(decision_id, summary, manifest) -> ReceiptMeta` builds the `[meta]` block; `DecisionRecorder::complete_with_receipt(conn, status, counts, summary, receipt: Option<&T>)` writes the receipt (if a path was set up) then completes — write failure becomes a warning. `recorder.decision_id()` exposes the inserted ID; `recorder.receipt_abs_path()` / `recorder.receipt_ref()` expose receipt location. **`decision.rs` depends on `receipt.rs`, never the reverse** — `receipt.rs` stays decision-agnostic.
- `receipt.rs` - Receipt format + writers (knows nothing about decisions). `ReceiptRef` (root_id + rel_path), `ReceiptMeta` (shared TOML `[meta]`), `ReceiptPlacement` enum (`Targeted { archive_root_id, archive_root_path, base_dir_rel }` for apply — mirrors the destination per `layout`; `LedgerRoot { root_id, root_path }` for non-targeted exclusion receipts — flat at the ledger root). Receipt bodies: `ApplyReceipt`/`ApplyReceiptItem`; `ExcludeReceipt`/`ExcludeReceiptItem` (set/clear/single-source); `DuplicatesReceipt`/`DuplicateGroup`/`DuplicateKeptEntry`/`DuplicateExcludedEntry` (kept omits `previous_decision_id`); `ObjectExcludeReceipt`/`ObjectExcludeEntry`/`ObjectSourceReceiptEntry`. Path helpers: `receipt_filename(decision_id, command)` (6-digit zero-padded), `compute_targeted_receipt_rel_path(decision_id, command, base_dir_rel, layout)`, `compute_ledger_root_receipt_rel_path(decision_id, command)` (flat `.canon-ledger/{filename}`). `resolve_ledger_root(roots, config) -> Option<(i64, String)>` picks the archive root that holds non-targeted receipts (config.root if an active archive, else lowest-id active archive, else None). Writers: `write_receipt(path, receipt, comment_summary)` (writes `.incomplete`), `finalize_receipt(path)` (renames `.incomplete` → `.toml`).
...
**Decision recording** (`ops/decision.rs`): The `DecisionRecorder` provides two-phase recording — `start()` INSERTs a "started" record, `complete()` UPDATEs with outcome and (for receipts) finalizes the `.incomplete` file to `.toml`. `complete_with_receipt()` folds "write receipt then complete" into one call (shared by exclude and apply). Execute functions accept `Option<&DecisionParams>` — `None` skips recording (used in tests), `Some` enables it. The recorder catches its own errors by collecting warnings in a `Vec<String>` (never writes to stderr — that's the interface's job). The exclude execute functions **drain `take_warnings()` into their result struct's `warnings` field**, and the interface prints them (apply does the same via `ApplyResult.warnings`); a 0-item plan skips recording entirely (no decision row, no dangling receipt). For commands without a single execute function (scan, cluster, import-facts), the interface creates the recorder and wraps the operation calls.

**Receipt writing** (`ops/receipt.rs`): Both apply and exclude pass a `ReceiptPlacement` to `DecisionRecorder::start()`, which computes the receipt path, creates the directory, registers the path in the DB, and stores the abs path. **Apply** builds `ReceiptPlacement::Targeted` (from `ApplyExecuteParams.receipt_ctx`); the receipt mirrors the destination under `.canon-ledger/` per `layout`. **Exclude** resolves `ReceiptPlacement::LedgerRoot` via `resolve_ledger_root()` in `src/exclude.rs` (warning, when receipts are enabled, if no archive root exists); exclusion receipts are flat at the ledger root's `.canon-ledger/`, independent of layout. The execute function builds the matching body (`ApplyReceipt` / `ExcludeReceipt` / `DuplicatesReceipt` / `ObjectExcludeReceipt`) and calls `recorder.complete_with_receipt(...)`, which writes the `.incomplete` then finalizes to `.toml`. Apply items record per-transfer: `source_root`, `source_rel_path`, `destination_rel_path`, `hash`, `size`, `mtime`, `previous_decision_id`. Exclusion items record `root`, `rel_path`, `hash`, `size`, `mtime`, `previous_decision_id` (duplicates group by hash into kept/excluded; object exclusion lists every source sharing the content, all roles).

**`previous_decision_id`**: For apply, before each transfer `execute_single_transfer()` calls `repo::source::fetch_decision_id_at_path()` to read the existing destination record's `decision_id`. For exclusion, the predecessor is captured at plan time on each `ExcludeItemData` (the source's `decision_id` before the op). Either way it is stored in the receipt item as `previous_decision_id`: because that id is also the predecessor receipt's filename stem, the chain is walkable from disk alone.
